...
//		startDate = sdf.format(c.getTime());
     ...
     * @return
     */
    public static Date getFirstDayOfMonthNew(String yearMonth) {
        String[] tmp = yearMonth.split("-");
        int year = Integer.parseInt(tmp[0]);
        int month = Integer.parseInt(tmp[1]);
        Calendar cal = Calendar.getInstance();
     ...
     * @return
     */
    public static Date getLastDayOfMonthNew(String yearMonth) {
        String[] tmp = yearMonth.split("-");
        int year = Integer.parseInt(tmp[0]);
        int month = Integer.parseInt(tmp[1]);
        Calendar cal = Calendar.getInstance();
    ...
    public static int getDaysBetween(Date startDate, Date endDate) {
        ...
        try {
            ...
            between_days = (endTimes - startTimes) / (60 * 60 * 1000 * 24);
        } catch (ParseException e) {
            e.printStackTrace();
        }
     ...
     */
    public static int compareDates(String date_str1, String date_str2) {
        try {
            Date date1 = DateUtil.parseDate(DateUtil.DATE_TIME_PATTERN, date_str1);
            Date date2 = DateUtil.parseDate(DateUtil.DATE_TIME_PATTERN, date_str2);
     ...
     * @date 2016-6-7 下午15:28:20
     */
    public static Time parseSqlTime(String date_str) {
        Time time = null;
        try {
            SimpleDateFormat timeHmsFormat = new SimpleDateFormat(TIME_PATTERN);
            if (null != timeHmsFormat) {
                time = new Time(timeHmsFormat.parse(date_str).getTime());
            }
        } catch (Exception e) {
